...
def calculate_demographic_data(print_data=True):
    ...
    higher_education = ( (df['education'] == 'Bachelors') | (df['education'] == 'Masters') 
               | (df['education'] == 'Doctorate') )
    lower_education = ( (df['education'] != 'Bachelors') & (df['education'] != 'Masters') 
        & (df['education'] != 'Doctorate' ) )

    # percentage with salary >50K
    higher_education_rich = round(100 * df.loc[higher_education & (df['salary'] == '>50K')].shape[0] 
                                  / df.loc[higher_education].shape[0], 1)
    lower_education_rich = round(100 * df.loc[lower_education & (df['salary'] == '>50K')].shape[0] 
                                 / df.loc[lower_education].shape[0], 1)
    ...
    rich_percentage = round(100 * df.loc[num_min_workers & (df['salary'] == '>50K')].shape[0] 
                        / df[num_min_workers].shape[0], 1)
    ...
    top_IN_occupation = df.loc[cond, 'occupation'].mode()[0]
